...
#   anthropic/claude-opus-4-8    (highest quality where available, most expensive)
...
openclaw models auth setup-token --provider <provider>
openclaw config set agents.defaults.model "<provider>/<model>"
# Current Sonnet is a strong default for coding — fast, capable, reasonable cost
# Opus 4.8 where available for highest quality (expensive), Haiku for speed
...
```bash
# Adaptive thinking (current Claude models)
openclaw config set agents.defaults.params.thinkingLevel "adaptive"
