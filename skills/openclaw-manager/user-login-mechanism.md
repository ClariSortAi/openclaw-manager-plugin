...
`v2026.4.21+` note: when `commands.enforceOwnerForCommands=true`, wildcard channel `allowFrom` no longer acts as owner fallback; configure explicit owner ids via `commands.ownerAllowFrom` or ensure owner/admin identity mapping is available.
