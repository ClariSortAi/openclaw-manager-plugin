...
`v2026.5.7+` note: `openclaw channels list` is channel-only by default; use `openclaw channels list --all` for bundled/catalog channel inventory and `openclaw models auth list` for saved model auth profile status.
