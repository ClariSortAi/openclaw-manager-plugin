...
`v2026.5.5+` note: LINE `dmPolicy: "open"` must include wildcard `allowFrom`; iOS private LAN or `.local` pairing routes may use `ws://`, while Tailscale/public routes stay on `wss://`.
