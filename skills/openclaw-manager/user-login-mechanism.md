...
`v2026.4.5+` note: legacy public alias keys (for example `talk.voiceId`/`talk.apiKey` and legacy `allow` toggles) are removed from canonical schema; run `openclaw doctor --fix` and then re-validate config.
