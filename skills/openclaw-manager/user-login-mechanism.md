...
`v2026.4.23+` note: SecretRef-backed webhook route secrets are also re-resolved per request, so old webhook secrets are revoked after `openclaw secrets reload` without a gateway restart.
