...
`v2026.4.5+` note: legacy public config aliases (for example `talk.voiceId`, `talk.apiKey`, and `agents.*.sandbox.perSession`) were removed from canonical validation. Run `openclaw doctor --fix` and `openclaw config validate` to rewrite/verify canonical keys.
