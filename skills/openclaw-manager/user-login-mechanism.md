...
`v2026.4.29+` note: restrictive `messaging`/`minimal` tool profiles are not widened by `tools.exec` or `tools.fs` sections; add explicit `alsoAllow` entries for any intentional tool exposure.
