...
`v2026.4.5+` note: additional legacy public config aliases were removed from the active schema; use canonical keys and run `openclaw doctor --fix` post-upgrade.
`v2026.4.9+` note: `openclaw doctor` now provides explicit reauth commands when provider auth checks fail, which helps recover stale login/token state faster.
