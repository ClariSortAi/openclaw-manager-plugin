...
As of v2026.5.26-v2026.5.27, Slack final replies survive late cleanup, downloaded files stay out of reply media, and sender/requester checks are tighter across approval and fallback-warning paths.
...
`v2026.5.26+` reliability note: WhatsApp restores group/media behavior, updates Baileys, and supports thumb reaction approvals so mobile approval flows do not need textual `/approve` commands.
...
In v2026.5.19-v2026.5.27, Telegram forum-topic lanes prevent one busy topic from blocking siblings, progress/typing context is preserved across replies, and `sendMessage` action replies use durable outbound delivery.
...
- v2026.5.26+ treats targeted `/command@TargetBot` entities as explicit mentions for addressed bots, improving `requireMention` groups.
...
As of v2026.5.20-v2026.5.27, Discord voice sessions can follow configured users into voice channels, preserve realtime turn context, prebuffer playback to reduce choppy starts, bucket large model-picker menus, and apply tighter guild requester checks.
...
In v2026.5.26-v2026.5.27, legacy iMessage gained attachment-root/media fixes, reaction approvals, duplicate local-source suppression, and duplicate native exec approval-send suppression. BlueBubbles remains the recommended path for full feature support.
...
v2026.5.27 suppresses thread sends in Google Chat DMs so direct-message replies do not get routed into unavailable thread contexts.
