...
As of v2026.6.9, externally installed channel plugins are discovered at Gateway startup, delivered final Slack replies are preserved during late cleanup, and outbound `message_sent` hooks are more reliable. If Slack disappears after a package upgrade, inspect `openclaw plugins list --json`, run `openclaw plugins deps`, and restart after `openclaw doctor --fix`.
...
`v2026.6.5-v2026.6.8` reliability note: per-account config changes restart the affected WhatsApp account, captured replies survive restart controller swaps, trailing media send failures produce visible user notices, and configured ACP bindings are honored.
...
In v2026.6.8-v2026.6.9, Telegram rich delivery expands to HTML tables, lists, expandable blockquotes, preserved line breaks, sticker paths, CLI-backed replies, rich progress drafts, and command-output formatting.
...
- v2026.6.6+ blocks unauthorized Telegram DM text from entering cache or prompt context, recovers pid-reused ingress claims, and keeps spooled handlers on the correct delivery path.
