...
As of v2026.6.8, outbound Slack delivery emits `message_sent` hooks for plugin/reply-payload integrations, and same-channel generated media completions/final replies preserve the intended channel context more reliably.
...
`v2026.5.26+` approval note: WhatsApp thumb reactions can approve pending actions on mobile-friendly flows.
`v2026.6.8+` ACP note: WhatsApp honors configured ACP bindings and preserves account-scoped DM send policy more reliably after restart/recovery paths.
...
In v2026.6.8+, Telegram rich-message delivery preserves tables, lists, expandable blockquotes, intentional line breaks, CLI-backed replies, and richer final replies. If formatting degrades after upgrade, repair plugin deps and probe with `openclaw channels status --channel telegram`.
...
- v2026.6.6+ prevents unauthorized DM text from entering cache or prompt context and keeps callbacks/draft chunks coherent during isolated polling delivery.
...
In v2026.6.5+, Matrix voice-message preflight and thread-aware read/reply behavior preserve context for voice notes and threaded conversations.
...
In v2026.6.5+, native approval card actions and click handling let Google Chat approval flows use platform-native cards instead of generic message flow.
...
- v2026.6.5-v2026.6.8 improves Feishu streaming-card content preservation, transient rate-limit retries, dynamic-agent binding reuse, and P2P direct-message thread replies.
