...
### Slack Interactive Reply Directives (v2026.3.13+; expanded in v2026.3.24 and approvals in v2026.3.31-v2026.4.8)
...
As of v2026.4.7, you can require explicit mentions inside bot-participated threads using `channels.slack.thread.requireExplicitMention`.
As of v2026.4.8, Socket Mode connections respect ambient `HTTP_PROXY`/`HTTPS_PROXY` with `NO_PROXY` exclusions for proxy-only deployments.

### Slack Thread Mention Hardening (v2026.4.7+)

Use this in high-volume workspaces to avoid accidental thread wake-ups after the bot has already participated:

```bash
openclaw config set channels.slack.thread.requireExplicitMention true
```

### Slack Context Visibility Filtering (v2026.4.5+)

Per-channel `contextVisibility` can reduce accidental quote/thread history leakage in mixed-trust rooms:

```bash
# Options: all | allowlist | allowlist_quote
