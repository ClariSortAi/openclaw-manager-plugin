...
As of v2026.5.4+, `streaming.progress.render: "rich"` can render richer Slack Block Kit progress drafts, and `agents.defaults.toolProgressDetail: "raw"` can expose raw command/detail output while debugging.
As of v2026.5.7+, handled top-level channel turns in implicit-conversation Slack channels can route to thread-scoped sessions when Slack reply threading is enabled, preserving root/thread continuity.
...
`v2026.5.7+` reliability note: proactive phone-number sends use Baileys LID forward mappings when available, captioned `MEDIA:` auto-replies avoid duplicate empty media messages, and Telegram-style visible-reply enforcement is applied to WhatsApp group/channel turns.
...
In v2026.4.29+ Telegram uses durable message edits for streaming previews to reduce draft-to-message flicker. In v2026.5.3+, `streaming.mode: "progress"` can enable shared progress-draft behavior with auto labels. In v2026.5.7+, `accessGroup:*` allowlists are honored for DMs, groups, native commands, and callbacks before numeric sender-id checks.
...
As of v2026.5.7+, use `openclaw channels capabilities --channel discord --target channel:<id>` and `openclaw channels status --probe` to audit voice-channel permissions, including auto-join targets, before `/vc join`. Discord voice capture also has a longer default post-speech silence grace and `voice.captureSilenceGraceMs` for noisy rooms.
