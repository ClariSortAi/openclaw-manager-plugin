...
As of v2026.6.8, Slack outbound `message_sent` hooks, same-channel final reply mirroring, plugin health/status reporting, and managed plugin drift diagnostics improve upgrade triage for externalized Slack installs.
...
`v2026.6.8+` ACP note: configured WhatsApp ACP bindings are honored, and recent stable fixes preserve profile auth roots, reconnect behavior, media error reporting, and captured replies across restarts.
...
In v2026.6.8+, Telegram rich delivery expands to structured text with tables, lists, expandable blockquotes, preserved intentional line breaks, rich final replies, and CLI-backed replies.
...
- v2026.6.8+ improves rich Markdown/HTML delivery, callback sequencing, thread-create CLI remapping, and group/media chunking around surrogate pairs and unbounded chunk limits.
...
`v2026.6.5+` reliability note: Matrix voice-message preflight and thread-aware read/reply behavior preserve more conversation context for voice notes and threaded rooms.
...
`v2026.6.5+` approval note: Google Chat approvals can render as native card actions instead of falling back to generic message flow.
