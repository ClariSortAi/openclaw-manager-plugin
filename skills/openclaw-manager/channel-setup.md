...
As of v2026.5.22-v2026.5.27, Slack delivery suppresses reasoning payloads before reply delivery/accounting, keeps downloaded read-only files out of reply media, retains delivered final replies during late cleanup, and benefits from deferred full Slack startup for faster Gateway startup.
...
`v2026.5.26+` reliability note: WhatsApp reaction approvals support thumb-based approval flows, group/media behavior received focused repair, and acknowledgement identity handling is more stable.
...
In v2026.5.22-v2026.5.27, Telegram preserves inbound text entities, forum-topic names, targeted `/command@TargetBot` mentions, native progress callbacks, and structured attachment media in `sendMessage` actions; durable outbound delivery reduces lost action replies.
...
- v2026.5.22+ adds `channels.telegram.pollingStallThresholdMs` handling in the isolated polling path so silent workers restart instead of wedging inbound updates.
...
`v2026.5.26+` Discord note: voice playback, wake replies, model picker pagination, media caption merging, proxy-aware metadata lookups, and numeric channel sends were improved. Large provider/model lists now use bucketed selection instead of endless prev/next paging.
...
In v2026.5.27+, untrusted Teams service URLs and invalid Teams attachment-fetch DNS targets are blocked earlier, while TeamsSDK patterns stay aligned with the hardened trust checks.
