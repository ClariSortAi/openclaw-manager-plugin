...
### Slack Interactive Reply Directives (v2026.3.13+; expanded through v2026.4.9)
...
As of v2026.4.8, Slack Socket Mode respects ambient HTTP(S) proxy and `NO_PROXY` settings, which fixes proxy-only deployments.
As of v2026.4.9, Slack private media downloads are hardened to keep bearer auth on same-origin `files.slack.com` redirects while still stripping it on cross-origin CDN hops.
