...
As of v2026.4.21, runtime outbound sends preserve explicit `threadTs` aliases more reliably, so automation and generic tool sends stay in the intended Slack thread.
...
`v2026.4.22+` behavior note: WhatsApp supports configurable native reply quoting via `replyToMode`, and per-group/per-direct `systemPrompt` policies are injected into inbound context (with `"*"` wildcard fallback and account-scoped override maps).
