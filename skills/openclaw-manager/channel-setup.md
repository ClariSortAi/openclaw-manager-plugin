...
As of v2026.5.19-v2026.5.22, Slack duplicate/thread recovery fails closed when same-channel replies lose thread context, native plugin approvals stay in the originating app conversation thread, and reasoning payloads are suppressed before Slack reply delivery and dispatch accounting.
...
`v2026.5.18-v2026.5.22` delivery note: outbound image/GIF/video document sends preserve original bytes when `forceDocument`/`asDocument` is requested, `upload-file` lowers through the normal media transport, pending outbound deliveries drain periodically while connected, and inbound delivery state is persisted before dispatch so retryable failures can redeliver safely.
...
In v2026.5.18-v2026.5.22, Telegram topic-aware lanes keep forum topics from blocking siblings, retries fail closed instead of falling back to base chats when a thread is missing, markdown tables/code-block languages are preserved, local `path`/`filePath` attachments send through `sendMessage`, and polling stall thresholds can restart silent workers.
...
- v2026.5.22+ keeps `:topic:` / `:topicId` delivery targets on the Telegram plugin path for cron and ACP, records bot-authored replies for prompt context, and dedupes replayed isolated-ingress messages by Telegram chat/message identity.
...
Discord voice sessions can follow configured users into allowed voice channels with bounded reconciliation, DAVE recovery preservation, and default realtime wake-name gating based on the agent name. Long-running component workflows can set a bounded `agentComponents.ttlMs` callback registry lifetime with per-account overrides and a 24-hour cap.
