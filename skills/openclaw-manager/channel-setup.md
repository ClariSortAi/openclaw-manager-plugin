...
As of v2026.5.19-v2026.5.27, Slack duplicate-thread safeguards, downloaded-file media filtering, delivered-final preservation, reasoning previews, and late-cleanup handling are stronger. If a same-channel threaded reply loses thread context, current stable fails closed instead of posting at channel root.
...
# v2026.5.12+: probe only WhatsApp without waking unrelated monitors
...
`v2026.5.20-v2026.5.27+` reliability note: WhatsApp group-drop diagnostics, ack identity, media/document filenames, profile auth roots, QR display, and thumb-reaction approvals are steadier in current stable.
...
In v2026.5.18-v2026.5.27+, Telegram topic lanes, forum-topic names, reply context, targeted `/command@TargetBot` mentions, durable `sendMessage` actions, native progress callbacks, and ENETDOWN retry handling reduce stalls and topic misdelivery.
...
- v2026.5.27+ preserves Telegram SecretRef prompt config and polling keepalives more reliably; if a configured bot stops after credential rotation, repair with `openclaw doctor --fix` and then probe only Telegram with `openclaw channels status --channel telegram`.
...
### Discord Voice and Progress (v2026.5.18+)

Current stable improves Discord voice following, realtime playback, wake matching, large model-picker buckets, progress-mode final replies, and suppression of recovered tool-warning artifacts from successful replies. If voice sessions or progress drafts behave inconsistently, upgrade to v2026.5.27+ and re-check bot permissions, voice channel allowlists, and configured model picker defaults.
...
In v2026.5.27+, untrusted Microsoft Teams service URLs are blocked while TeamsSDK patterns remain aligned; if delivery starts failing after an Azure/Bot Framework endpoint change, verify the service URL and tenant trust configuration before relaxing network policy.
