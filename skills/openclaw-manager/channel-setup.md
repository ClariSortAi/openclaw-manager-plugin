...
As of v2026.6.1-v2026.6.8, Slack final replies and native approval routes are more resilient during late cleanup/restart paths, and outbound `message_sent` hooks are preserved for plugin integrations.
...
`v2026.5.28-v2026.6.8` reliability note: profile-scoped WhatsApp auth roots, QR display/retry behavior, per-account config restarts, captured replies after restart, media error reporting, and ACP binding delivery all received fixes. If a WhatsApp account is linked but replies go to the wrong context, verify the active account/profile config and upgrade before relinking.
...
In v2026.6.6-v2026.6.8, Telegram account-scoped topics route to the correct agent, streamed text survives tool calls, callbacks and draft chunks stay coherent, unauthorized DM text stays out of cache/prompt context, and rich final replies can render structured tables, lists, expandable blockquotes, and intentional line breaks more faithfully.
...
- v2026.6.6+ avoids caching unauthorized DM text or leaking it into prompt context; if access-control behavior looks surprising, validate pairing/allowlist policy and inspect single-channel status before broad restarts.
...
### Current iMessage Reliability Notes

Legacy iMessage remains deprecated, but v2026.5.28-v2026.6.6 tightened existing users' restart behavior with always-on inbound recovery, durable echo markers, block streaming, idle approval discovery, and typing continuity during tool work. Prefer migrating to BlueBubbles for new setups.
...
In v2026.6.5+, Matrix voice-message preflight and thread-aware read/reply handling improve room/thread context preservation.
...
As of v2026.6.5+, Google Chat approvals can use native card actions and click handling instead of generic message flow.
