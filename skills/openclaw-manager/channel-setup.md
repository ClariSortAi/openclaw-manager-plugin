...
As of v2026.5.4+, `streaming.progress.render: "rich"` can render Block Kit progress drafts with structured progress lines.
In v2026.5.12 beta, Slack adds `unfurlLinks`, `unfurlMedia`, and `replyBroadcast` controls for bot replies and thread broadcasts, including per-account overrides.
...
`v2026.5.4+` reliability note: WhatsApp setup canonicalizes phone allowlists to digit-only WhatsApp ids while still accepting E.164/JID/`whatsapp:` inputs, and proactive phone-number sends can use Baileys LID forward mappings when available.
...
In v2026.5.7+, `accessGroup:*` allowlists apply consistently across DMs, groups, native commands, and callback authorization before Telegram numeric sender-id checks.
In v2026.5.12 beta, Telegram polling ingress can run in an isolated worker with a durable local spool, and `requireMention` skips unmentioned group media before download.
...
### Discord Voice Diagnostics (v2026.5.4+)

Use channel probes before enabling voice automation:
...
Recent stable releases surface missing Connect/Speak/Read Message History permissions for configured auto-join voice targets and expose `voice.captureSilenceGraceMs` tuning for noisy sessions.
