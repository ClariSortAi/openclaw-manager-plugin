...
| Session transcript redaction | Applies configured redaction patterns to persisted session JSONL transcript text, not just live log sinks | v2026.4.25 |
| Device token rotation scope containment | Enforces caller-scope containment for token rotate/revoke paths and stops shared/admin rotation responses from echoing rotated bearer tokens | v2026.4.25-v2026.4.26 |
| Health probe route reservation | Reserves `/healthz` and `/readyz` before plugin, canvas, and Control UI route stages so liveness/readiness probes remain available if later handlers stall | v2026.4.25 |
| Plugin install scanner hardening | Keeps plugin install scans fail-closed while skipping packaged test files, force-scanning declared runtime entrypoints, and only allowing exact trusted host-package peer links | v2026.4.26 |
| Sub-agent allowlist enforcement | Enforces `subagents.allowAgents` even for explicit same-agent `sessions_spawn(agentId=...)` requests | v2026.4.26 |
| Reasoning-output privacy boundary | Treats malformed/orphan reasoning tags as a delivery/history sanitization boundary so local-model chain-of-thought text is not leaked | v2026.4.26 |
...
- [ ] If using plugin workflows, inspect/repair persisted plugin registry state with `openclaw plugins registry --refresh` or `openclaw doctor --fix` after upgrades to v2026.4.25+
- [ ] If using diagnostics exports, confirm OpenTelemetry/Prometheus settings stay bounded and do not include prompts, tool outputs, command text, raw request IDs, or recipient/message content
