...
| Skill execution compatibility removal | Old `cat SKILL.md && printf ... && <skill-wrapper>` allowlist compatibility is removed so skills load through read-tool paths and only the real executable is auto-allowed | v2026.5.20 |
| Secret file symlink fail-closed behavior | Credential loaders using `rejectSymlink: true` refuse symlinked secret files for Telegram, LINE, Zalo, IRC, Nextcloud Talk, and related token paths | v2026.5.20 |
| Group prompt boundary hardening | Untrusted group prompt metadata is routed outside system prompts so channel/group context cannot silently become privileged instruction text | v2026.5.27 |
| No-auth Tailscale exposure rejection | Tailscale exposure without authentication is rejected instead of relying on implicit local/trusted assumptions | v2026.5.27 |
| Side-effecting command and Node env hardening | Side-effecting command wrappers and unsafe Node runtime environment overrides are blocked in approval/runtime paths | v2026.5.27 |
| Teams service URL trust checks | Untrusted Microsoft Teams service URLs are blocked before fetch/delivery paths use them | v2026.5.27 |
| Device/node role admin gating | Node/device-role approvals require admin authority instead of lower-privilege requester contexts | v2026.5.27 |
| Browser token expiry after auth rotation | Browser tokens expire after gateway auth rotation so stale browser credentials do not survive credential changes | v2026.5.28 |
| MCP redirect and global config protection | MCP HTTP redirects are guarded, global agent config defaults are protected, and malformed operational limits stay bounded | v2026.6.5 |
| Admin-only HTTP override surfaces | HTTP session/model override surfaces require admin privileges | v2026.6.8 |
| Hono runtime patch | Published OpenClaw and ACPX packages include patched Hono 4.12.25 runtime | v2026.6.8 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.6.8+ for latest externalized-plugin repair, Telegram/WhatsApp reliability, Skill Workshop, SQLite state migrations, Codex/OpenAI auth, Gateway protocol, and security/provenance hardening)
...
- [ ] If upgrading from pre-v2026.5.18, upgrade Node to v22.19.0+ before updating OpenClaw
- [ ] If using custom skills or approval wrappers, remove legacy `cat SKILL.md && printf ...` compatibility assumptions and verify skills load through supported read-tool paths
- [ ] If using Tailscale or Teams, verify no-auth Tailscale exposure is rejected and Teams service URLs are from trusted sources
- [ ] If relying on key-free web search, explicitly configure the intended provider; v2026.6.8+ does not auto-select Parallel Free, DuckDuckGo, Ollama, or Codex Hosted Search
- [ ] After v2026.6.x SQLite migrations, run `openclaw doctor --fix` and verify cron/plugin state before relying on scheduled automations
...
- Use modern, instruction-hardened models (Opus 4.8 / Sonnet 4.7 with adaptive thinking)
