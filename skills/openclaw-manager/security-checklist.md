...
| Workspace dotenv provider credential isolation | Workspace `.env` provider credentials are ignored for trusted auth paths, reducing accidental credential steering from untrusted workspaces | v2026.5.28 |
| Teams service URL trust | Untrusted Teams service URLs and invalid Teams attachment-fetch DNS targets are rejected earlier | v2026.5.28 |
| Phone-control mutation authorization | Phone-control mutation paths require tighter authorization before state changes are accepted | v2026.5.28 |
| Directive persistence authorization | Provider and relayed-channel metadata are preserved so directive persistence keeps the intended channel-originated authority context | v2026.5.28 |
| Browser and config input bounds | Browser tab indexes, viewport sizes, timeout values, cookie expiries, Gateway ports, cron epochs, provider retry headers, and malformed numeric/version inputs fail closed earlier | v2026.5.28 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.5.28+ for latest externalized-plugin repair, channel/provider reliability, ClawPDF encrypted PDF support, official Copilot/Tokenjuice packaging, and security/input hardening)
...
- [ ] If using externalized official channels/providers (Slack, WhatsApp, Amazon Bedrock, Anthropic Vertex) or official install-on-demand plugins (Copilot, Tokenjuice), run `openclaw plugins deps` and `openclaw doctor --fix` after upgrade
...
- [ ] If using Teams, verify service URL and attachment-fetch DNS targets are trusted after upgrading to v2026.5.28+
- [ ] If relying on workspace `.env` provider credentials, move trusted provider auth into configured auth profiles or SecretRefs before upgrading to v2026.5.28+
