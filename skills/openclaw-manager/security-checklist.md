...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest hardening and recovery tooling, prefer **v2026.4.23+**.
...
| Workspace env runtime-control blocking | Blocks all `OPENCLAW_*` keys from untrusted workspace `.env` files so new runtime-control variables fail closed instead of being inherited silently | v2026.4.20 |
| Owner-only command authorization | Requires owner identity or internal `operator.admin` for owner-enforced commands, so wildcard `allowFrom` and empty owner-candidate fallbacks cannot grant command access | v2026.4.21 |
| Teams audience-token replay protection | Requires shared Bot Framework audience tokens to name the configured Teams app via verified `appid` or `azp`, blocking cross-bot token replay | v2026.4.23 |
| Webhook SecretRef hot reload | Re-resolves `SecretRef`-backed webhook route secrets per request so `openclaw secrets reload` revokes previous webhook secrets without a gateway restart | v2026.4.23 |
| Paired-device scope restrictions | Restricts non-admin paired-device sessions to their own pairing list, approval, and rejection actions instead of global device-management visibility | v2026.4.23 |
| Gateway websocket broadcast scope gates | Requires operator scopes for chat, agent, tool-result, and unknown broadcast events so pairing/node sessions cannot passively receive session content | v2026.4.23 |
| Agent gateway config allowlist | Narrows model-driven `gateway config.apply`/`config.patch` mutations to safe prompt/model/mention-gating paths and keeps auth/TLS, sandbox, hooks, MCP, plugin trust, and filesystem hardening operator-only | v2026.4.23 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.4.23+ for latest provider, channel, diagnostics, and security hardening)
...
- [ ] After rotating gateway auth token/SecretRef, verify HTTP surfaces (`/v1/*`, `/tools/invoke`, plugin routes) require the new bearer without waiting for a gateway restart (v2026.4.23+)
- [ ] After rotating webhook `SecretRef`s, verify webhook routes reject the previous secret after `openclaw secrets reload` without waiting for a full gateway restart (v2026.4.23+)
- [ ] Workspace `.env` files do not contain `OPENCLAW_*` runtime-control variables; current builds fail closed on these keys (v2026.4.20+)
...
### Mitigations
- Keep OpenClaw updated to latest version (minimum v2026.3.1, recommended v2026.4.23+)
