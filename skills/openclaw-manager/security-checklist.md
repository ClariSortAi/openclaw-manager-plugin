...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest hardening and recovery tooling, prefer **v2026.4.21+**.
...
| Workspace `OPENCLAW_*` env key protection | Blocks all `OPENCLAW_*` keys from untrusted workspace `.env` files so workspace-local env loading fails closed for runtime-control variables | v2026.4.20 |
| Device pairing scope restriction | Non-admin paired-device sessions (device-token auth) restricted to their own pairing list, approve, and reject actions; admin and shared-secret sessions retain full visibility | v2026.4.20 |
| Extended gateway tool config mutation guard | Model-driven `config.patch`/`config.apply` cannot rewrite operator-trusted paths (sandbox, plugin trust, gateway auth/TLS, hook routing/tokens, SSRF policy, MCP servers, workspace filesystem hardening), including via `agents.list[]` edits | v2026.4.20 |
| WebSocket broadcast scope gating | Chat, agent, and tool-result event frames require `operator.read` or higher; pairing-scoped and node-role sessions no longer passively receive session chat content | v2026.4.20 |
| Forwarded-header reverse-proxy protection | Any forwarded-header evidence (`Forwarded`, `X-Forwarded-*`, `X-Real-IP`) is treated as proxied traffic before pairing locality checks, preventing reverse-proxy topologies from using the loopback shared-secret auto-pairing path | v2026.4.20 |
| QQBot SSRF guard | SSRF guard added to direct-upload URL paths in QQBot `uploadC2CMedia` and `uploadGroupMedia` | v2026.4.20 |
| Telegram group model picker authorization | `/models` group model-picker callbacks require the same authorization as direct commands so unauthorized group participants cannot browse or change the session model via inline buttons | v2026.4.20 |
| Owner-enforced command identity hardening | When `enforceOwnerForCommands=true`, commands now require actual owner identity; wildcard `allowFrom` or unset `commands.ownerAllowFrom` no longer grant access to non-owners | v2026.4.21 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.4.21+ for latest auth, interaction-allowlist, execution hardening, and WebSocket scope gating)
...
- [ ] If `enforceOwnerForCommands=true`, ensure `commands.ownerAllowFrom` is explicitly configured — wildcard `allowFrom` alone no longer grants owner-command access (v2026.4.21+)
- [ ] Workspace `.env` files do not rely on `OPENCLAW_*` env keys — these are blocked from workspace env loading for security (v2026.4.20+)
...
### Mitigations
- Keep OpenClaw updated to latest version (minimum v2026.3.1, recommended v2026.4.21+)
