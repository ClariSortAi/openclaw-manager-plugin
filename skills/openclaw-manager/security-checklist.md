...
| Host exec env sanitization expansion | Blocks additional Java/Rust/Cargo/Git/Kubernetes/cloud-credential/Helm env override pivots in host exec paths | v2026.4.7 |
| Fetch redirect payload stripping | Drops request bodies and body-describing headers on cross-origin `307`/`308` redirects by default to reduce secret-bearing SSRF redirect leakage | v2026.4.7 |
| Browser SSRF post-interaction rechecks | Re-runs forbidden-destination checks after click/evaluate/hook-driven main-frame navigations | v2026.4.9 |
| Dotenv runtime-control lockout | Blocks runtime-control and browser-control override env vars from untrusted workspace `.env` files; rejects unsafe URL-style browser control override specifiers | v2026.4.9 |
| Remote node exec-event trust boundaries | Marks node `exec.*` summaries untrusted and sanitizes node-provided command/output text before enqueueing | v2026.4.9 |
| Provider onboarding auth-choice isolation | Prevents untrusted workspace plugins from colliding with bundled provider auth-choice handlers during non-interactive onboarding | v2026.4.9 |
...
- [ ] If upgrading to v2026.4.5+, run `openclaw doctor --fix` and `openclaw config validate` to remove retired legacy aliases (`talk.voiceId`, `talk.apiKey`, `agents.*.sandbox.perSession`, `browser.ssrfPolicy.allowPrivateNetwork`)
