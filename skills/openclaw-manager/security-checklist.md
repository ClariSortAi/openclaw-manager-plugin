...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest hardening and recovery tooling, prefer **v2026.3.28+**.
...
| Qwen auth-path hardening | Removes deprecated `portal.qwen.ai` OAuth path (`qwen-portal-auth`) and requires Model Studio API-key-based flows | v2026.3.28 |
| Legacy config migration behavior | Drops automatic rewrites for config migrations older than two months; stale legacy keys now fail validation and require explicit cleanup | v2026.3.28 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.3.28+ for latest auth, install-flow, and execution hardening)
...
### Mitigations
- Keep OpenClaw updated to latest version (minimum v2026.3.1, recommended v2026.3.28+)
