...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest hardening and recovery tooling, prefer **v2026.4.29+**.
...
| `tools.exec`/`tools.fs` restrictive-profile widening removed | Configured `tools.exec` and `tools.fs` sections no longer implicitly widen `messaging` or `minimal` profiles; add explicit `alsoAllow` entries instead. A startup warning identifies affected configs. | v2026.4.29 |
| Payment credential redaction | Payment credential field names (card number, CVC/CVV, shared payment tokens) are redacted from logs and tool-payload UI events by default | v2026.4.29 |
| Additional cloud API key redaction | Tencent Cloud, Alibaba Cloud, HuggingFace, and Replicate API key patterns added to default log redaction rules | v2026.4.29 |
| Timing-safe credential comparison | Credential byte comparisons now use padded timing-safe buffers, closing timing side channels in shared-token and password auth paths | v2026.4.29 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.4.29+ for latest auth, interaction-allowlist, tools-profile, and execution hardening)
...
- [ ] After upgrading to v2026.4.29+, verify any `tools.exec` or `tools.fs` sections in restrictive profiles (`messaging`, `minimal`) — they no longer implicitly widen; add explicit `alsoAllow` entries if needed (startup warning will identify affected configs)
