...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest stable hardening and recovery tooling, prefer **v2026.6.1+**.
...
| Node runtime floor | Node 22 installs must be on 22.19+ so managed services, Docker images, and WSL shells do not run unsupported runtimes | v2026.5.18 |
| Browser snapshot SSRF enforcement | Browser snapshot reads validate tab URLs against SSRF policy before ChromeMCP or direct CDP access | v2026.5.26 |
| External content boundary wrapping | Fetched file text and metadata are wrapped as external content, and serialized tool-call text is scrubbed from replies | v2026.5.26 |
| No-auth Tailscale rejection | Gateway rejects no-auth Tailscale exposure instead of accepting an unsafe remote access posture | v2026.5.27 |
| Node/device-role admin approvals | Node and device-role approvals require admin authority | v2026.5.27 |
| Group prompt boundary separation | Untrusted group prompt metadata stays out of system prompts to reduce prompt-boundary confusion | v2026.5.27 |
| Plugin model pattern regex safety | Manifest model-pattern regexes are validated with a safe-regex compiler before matching | v2026.5.28 |
| Skill Workshop governance | Skill proposals use guarded apply/reject/quarantine/revision flows with rollback metadata and approval policy | v2026.6.1 |
| Auth/profile durability | Auth profiles are written atomically and migrated through durable state paths, with clearer force re-login recovery | v2026.6.1 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.6.1+ for latest Skill Workshop governance, SQLite-backed state, Codex/OpenAI recovery, channel reliability, and security/provenance hardening)
- [ ] Node runtime is v22.19.0+ everywhere OpenClaw runs (interactive shell, systemd/LaunchAgent/Scheduled Task, Docker/WSL, and update helpers)
...
- [ ] If using v2026.6.2+ prerelease install policy, review package/archive/source/upload/marketplace findings before changing trust settings; keep production guidance on stable releases unless beta behavior is intentional
...
- Use modern, instruction-hardened models (Opus 4.8 / current Sonnet 4.x with adaptive thinking)
