...
| Default remote auth rate limiting | Remote non-browser and HTTP gateway auth failures are rate-limited by default while preserving loopback exemptions | v2026.5.26 |
| Browser snapshot SSRF checks | Browser snapshot reads validate tab URLs against SSRF policy before CDP or ChromeMCP reads | v2026.5.26 |
| External-content prompt boundaries | Fetched file text and metadata are wrapped as external content, and group prompt metadata is kept out of system prompts | v2026.5.26-v2026.5.27 |
| Unsafe command/runtime blocking | Side-effecting command wrappers and unsafe Node runtime environment overrides are blocked before execution | v2026.5.27 |
