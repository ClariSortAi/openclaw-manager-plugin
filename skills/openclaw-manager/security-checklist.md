...
| Tool allowlist fail-closed controls | Preserves restrictive plugin-only tool allowlists, requires owner access for `/allowlist add|remove`, and fails closed when `before_tool_call` hooks crash | v2026.4.5 |
| Browser SSRF redirect bypass hardening | Blocks browser SSRF redirect bypass paths earlier in request handling | v2026.4.5 |
| Device and plugin-route scope tightening | Restricts paired-device token/scope escalation paths and keeps plugin route fallback scopes write-limited unless trusted-proxy callers explicitly declare narrower scopes | v2026.4.5 |
...
- [ ] If upgrading to v2026.4.5+, run `openclaw doctor --fix` and remove legacy alias keys (`talk.voiceId`, `talk.apiKey`, `agents.*.sandbox.perSession`, `browser.ssrfPolicy.allowPrivateNetwork`, `hooks.internal.handlers`, and channel/group/room `allow` toggles)
