...
| Owner-command identity enforcement | Requires owner identity (owner-candidate match or internal `operator.admin`) for owner-enforced commands instead of permissive wildcard/empty-owner fallback paths | v2026.4.21 |
| Bundled plugin runtime doctor repair | Extends `openclaw doctor --fix` to repair missing bundled plugin runtime dependencies in packaged installs | v2026.4.21 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.4.21+ for latest auth, owner-command, and plugin-repair hardening)
...
- [ ] If `enforceOwnerForCommands=true`, confirm owner candidates are explicitly configured and not relying on wildcard `allowFrom`/empty-owner fallbacks (v2026.4.21+)
- [ ] For packaged installs with missing bundled channel/provider runtime dependencies, run `openclaw doctor --fix` to trigger v2026.4.21+ runtime repair
