...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest hardening and recovery tooling, prefer **v2026.5.19+**.
...
| Trusted-proxy auth tightening | Rejects mixed shared-token trusted-proxy configs; same-host direct fallback requires the documented password path while token fallback stays rejected | v2026.3.31+ / v2026.5.19 |
...
| Browser current-tab URL enforcement | Applies current-tab URL allowlists to evaluate/batch and highlight routes while leaving tab-management actions unblocked | v2026.5.19 |
| Admin HTTP RPC instance binding | Binds plugin admin HTTP RPC dispatch to the accepting gateway instance so multi-gateway processes cannot execute control-plane calls against another live gateway | v2026.5.19 |
| Media decoder safety | Prevents unrecognized image bytes from invoking external decoder delegates and stops fallback chaining after real media-processing errors | v2026.5.18 |
| Control UI bootstrap redaction | Release validation now prevents unauthenticated bootstrap payloads from returning Control UI bearer tokens and keeps Docker harness ports loopback-only | v2026.5.18 |
| Browser/CDP diagnostics redaction | Keeps loopback proxy bypass active across both `NO_PROXY` casings and redacts home-relative Chrome MCP profile paths in attach-failure diagnostics | v2026.5.18 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.5.19+ for latest Node 22.19 floor, typed plugin tooling, browser/dialog handling, Telegram/Slack/Codex reliability, proxy CA support, and security/provenance hardening)
...
- [ ] If using browser automation, verify evaluate/highlight URL allowlists and handle `blockedByDialog` modal states instead of bypassing current-tab policy
- [ ] If using HTTPS managed forward proxies, pin the expected CA with `proxy.tls.caFile`
- [ ] If building custom Docker/Podman images, migrate package injection to `OPENCLAW_IMAGE_APT_PACKAGES` / `OPENCLAW_IMAGE_PIP_PACKAGES` and bind exposed harness/test ports to loopback unless intentionally published
...
### Mitigations
- Keep OpenClaw updated to latest version (minimum v2026.3.1, recommended v2026.5.19+)
