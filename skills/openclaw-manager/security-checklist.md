...
| Workspace dotenv runtime-control blocking | Blocks all untrusted workspace `.env` `OPENCLAW_*` keys so newly introduced runtime-control env vars cannot be inherited silently | v2026.4.20 |
| Owner-enforced command identity tightening | Requires true owner identity (`operator.admin` or owner-candidate match) for owner-enforced commands; wildcard `allowFrom`/empty candidates no longer satisfy owner checks | v2026.4.21 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.4.21+ for latest owner-command identity, dotenv runtime-control, auth, interaction-allowlist, and execution hardening)
...
- [ ] If `enforceOwnerForCommands=true`, validate explicit owner identity mappings (`commands.ownerAllowFrom`, owner candidates, or admin scopes); wildcard `allowFrom` is no longer a sufficient fallback in v2026.4.21+
- [ ] If you rely on workspace `.env`, ensure no required runtime-control values depend on `OPENCLAW_*` keys from untrusted workspace files (blocked in v2026.4.20+)
