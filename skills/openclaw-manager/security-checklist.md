...
| Secret-file symlink rejection | Credential loaders for Telegram, LINE, Zalo, IRC, Nextcloud Talk, and similar token files fail closed on symlinked secret files when `rejectSymlink` is requested | v2026.5.20 |
| Plaintext secret detection | `openclaw doctor` warns on plaintext secret-bearing config fields, including model provider API keys and sensitive provider headers | v2026.5.20 |
| Legacy skill-wrapper allowlist removal | Exec approvals no longer auto-allow `cat SKILL.md && printf ... && <skill-wrapper>` compatibility forms; skills must be read with the read tool and only real skill executables receive auto-allow treatment | v2026.5.20 |
| Locked package dependency graphs | The root npm package and OpenClaw-owned npm plugins ship generated shrinkwraps and require review for lockfile/shrinkwrap changes | v2026.5.22 |
| Workspace provider setup trust gating | Workspace provider plugins fail closed during setup-mode discovery unless explicitly trusted, preventing untrusted workspace code from running during provider setup | v2026.5.22 |
| Docker setup token redaction | Docker setup no longer prints the Gateway bearer token in setup logs or follow-up commands | v2026.5.22 |
| State-dir absolute path pinning | Relative `OPENCLAW_STATE_DIR` overrides are pinned to absolute paths at startup so later working-directory changes cannot retarget Gateway state | v2026.5.22 |
| Observability redaction | OpenTelemetry log bodies require explicit content capture, scoped agent/session keys are scrubbed from OTel and Prometheus labels, and diagnostic id attributes are reduced | v2026.5.22 |
| Diffs viewer XSS hardening | Diffs toolbar icons render from a closed icon-name map instead of raw HTML strings | v2026.5.22 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.5.22+ for latest externalized-plugin repair, Telegram/Discord/Codex reliability, Meeting Notes tooling, provider/auth performance, and security/provenance hardening)
- [ ] Node.js runtime is v22.19.0+ on Node 22, or preferably Node 24 for new installs (v2026.5.18+)
...
- [ ] If relying on skill-wrapper exec approval compatibility, update workflows so skill content is loaded with the read tool and only the real skill executable is approved (v2026.5.20+)
- [ ] Run `openclaw doctor --fix` and inspect plaintext-secret warnings after upgrading to v2026.5.20+; migrate inline provider keys/headers to SecretRef where possible
- [ ] If exporting diagnostics or telemetry, confirm OpenTelemetry and Prometheus label/content capture settings do not include raw session, message, chat, trace, tool-call, or secret identifiers (v2026.5.22+)
