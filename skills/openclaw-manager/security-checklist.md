...
| Memory prompt-injection filtering | Explicit `memory_store` input that looks prompt-like is rejected before embedding/storage, matching auto-capture filters | v2026.5.26 |
| Gateway auth rate limiting | Default auth rate limiting applies to remote non-browser and HTTP gateway auth failures when unset, while preserving loopback exemption | v2026.5.26 |
| Browser snapshot SSRF enforcement | Browser snapshot tab URLs are validated against SSRF policy before ChromeMCP or direct CDP reads | v2026.5.26 |
| External content boundary wrapping | System-event text is sanitized, fetched file text/metadata is wrapped as external content, and serialized tool-call text is scrubbed from replies | v2026.5.26 |
| Stale device-token rejection | RPCs from invalidated device-token clients are rejected during rotation | v2026.5.26 |
| Plugin model-pattern validation | Unsafe plugin manifest model-pattern regexes are ignored before matching | v2026.5.26 |
| Diffs toolbar XSS fix | Diffs viewer toolbar icons render from a closed icon-name map instead of HTML strings | v2026.5.22 |
| Group prompt content boundary | Untrusted group prompt metadata is routed outside system prompts | v2026.5.27 |
| Hostname normalization | Repeated trailing hostname dots are normalized before trust/routing decisions | v2026.5.27 |
| Side-effecting wrapper block | Side-effecting command wrappers are blocked in exec/runtime paths | v2026.5.27 |
| Unsafe Node runtime env override block | Unsafe Node runtime environment overrides are rejected | v2026.5.27 |
| Tailscale no-auth exposure rejection | Tailscale-exposed Gateway surfaces cannot rely on no-auth exposure | v2026.5.27 |
| Node/device-role approval authority | Node and device-role approvals require admin authority | v2026.5.27 |
| Teams service URL trust checks | Untrusted Microsoft Teams service URLs and invalid attachment-fetch DNS targets are blocked | v2026.5.27 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.5.27+ for latest channel/provider reliability, transcript support, Codex recovery, Gateway/status performance, and security/content-boundary hardening)
...
- [ ] If using Tailscale or reverse-proxy exposure, verify a supported auth mode is still explicitly configured; v2026.5.27+ rejects no-auth Tailscale exposure assumptions
- [ ] If host exec policy allows wrapper commands or custom Node runtime environment, re-test after v2026.5.27+ because side-effecting wrappers and unsafe Node env overrides now fail closed
- [ ] If relying on delegated/sub-agent context, verify required instructions are present in `AGENTS.md` / `TOOLS.md` or explicit config after v2026.5.22+ narrowed default bootstrap files
