...
| Input and service-boundary validation | Browser tool options, Gateway ports, cron epochs, channel retry timers, media content lengths, schema refs, Telegram callback pages, Teams attachment DNS targets, and malformed numeric/time values are rejected earlier | v2026.5.28 |
| Workspace/provider credential isolation | Workspace dotenv provider credentials are ignored and local setup-only channel loads from untrusted workspaces are blocked | v2026.5.28-v2026.6.1 |
| OAuth/token and remote media bounds | Unsafe OAuth/token lifetimes, retry-after delays, remote media overreads, trajectory export leaks, hooks-token auth reuse, and gateway WebSocket calls after close are rejected or redacted | v2026.6.1 |
| MCP redirect and config defaults hardening | MCP HTTP redirects are guarded, global agent config defaults are protected, and malformed operational limits stay bounded and explicit | v2026.6.5 |
| Fail-closed runtime boundaries | Transcript, sandbox, MCP, browser, channel, and exec-approval paths fail closed around unsafe access, timed-out approvals, and malformed boundary input | v2026.6.6 |
| Admin-only override surfaces | HTTP session/model override surfaces require admin privileges, reducing remote override risk | v2026.6.8 |
| Tool schema and model routing containment | Unreadable/post-hook OpenAI and Anthropic-family tool schemas are quarantined without broadening allowed tool choices | v2026.6.8 |
| Patched HTTP runtime dependency | Published OpenClaw and ACPX packages use Hono 4.12.25 patched runtime | v2026.6.8 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.6.8+ for latest channel delivery, Codex/provider recovery, official plugin repair, SQLite/state, model/search, usage footer, and security hardening)
- [ ] Node.js is v22.19.0 or later before updating to current packages (Node 24 recommended)
...
- [ ] If using Telegram account-scoped topics or rich final replies, verify unauthorized DMs stay out of prompt/cache context and rich text renders as intended after upgrade to v2026.6.8+
- [ ] If using key-free web-search providers, confirm they are explicitly enabled; current stable keeps Parallel Free, DuckDuckGo, Ollama, and Codex Hosted Search as deliberate opt-ins
- [ ] If using model/provider integrations, verify bounded model browsing and intended SecretRef-backed auth profiles with `openclaw models auth list` / `openclaw status --deep`
...
- Use modern, instruction-hardened models (for example Opus 4.8/Sonnet 4.7 with adaptive thinking)
