...
| Workspace `OPENCLAW_*` dotenv blocking | Blocks all `OPENCLAW_*` runtime-control keys from untrusted workspace `.env` files so cloned workspaces cannot steer gateway or CLI behavior | v2026.4.20 |
| Device and websocket scope tightening | Restricts non-admin device-token sessions to their own pairing operations and requires operator scopes for chat/tool-result websocket broadcasts | v2026.4.20 |
| Teams audience enforcement | Requires shared Bot Framework audience tokens to name the configured Teams app by verified `appid` or `azp`, blocking cross-bot token replay | v2026.4.23 |
| Plugin setup path hardening | Stops setup-api lookup from falling back to the launch directory, preventing workspace-local setup shims from executing during provider setup | v2026.4.23 |
| MCP owner-only tool policy | Applies owner-only tool checks and before-tool hooks on loopback `/mcp` `tools/list` and `tools/call`, matching HTTP invoke behavior | v2026.4.24 |
