...
The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, config backup permission hardening, SSRF DNS pinning, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. For latest hardening and recovery tooling, prefer **v2026.4.9+**.
...
| Browser SSRF post-navigation rechecks | Re-runs blocked-destination safety checks after interaction-driven browser navigations (`click`, `evaluate`, hook-triggered actions) so forbidden redirect destinations remain blocked | v2026.4.9 |
| Untrusted workspace `.env` guard expansion | Blocks runtime-control, browser-control override, and skip-server env vars from untrusted workspace `.env` files and rejects unsafe URL-style browser override specifiers | v2026.4.9 |
| Node exec event trust sanitization | Marks remote node `exec.*` summaries as untrusted and sanitizes command/output/reason text before enqueueing to prevent trusted `System:` injection | v2026.4.9 |
| Plugin auth-choice collision guard | Prevents untrusted workspace plugins from colliding with bundled provider auth-choice ids during non-interactive onboarding | v2026.4.9 |
| Dependency audit hardening | Pins `basic-ftp` to `5.2.1` for CRLF command-injection mitigation and updates core HTTP stack dependencies in production resolution paths | v2026.4.9 |
...
### Version & Patches
- [ ] Running v2026.3.1 or later (recommend v2026.4.9+ for latest auth, install-flow, and execution hardening)
...
- [ ] If upgrading to v2026.4.5+, run `openclaw doctor --fix` to migrate removed legacy public alias keys (`talk.voiceId`, `talk.apiKey`, legacy `allow` toggles, and related deprecated aliases)
...
- [ ] Untrusted workspace `.env` files are not relied on for runtime-control or browser-control overrides (blocked in v2026.4.9)
...
### Mitigations
- Keep OpenClaw updated to latest version (minimum v2026.3.1, recommended v2026.4.9+)
