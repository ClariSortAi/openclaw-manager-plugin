...
Always verify the user is running **v2026.3.1 or later**. Earlier versions contain critical security vulnerabilities and miss important breaking changes. The v2026.3.x line adds gateway auth bypass prevention, webhook auth enforcement, ACP sandbox inheritance, and macOS umask hardening on top of the 40+ fixes in v2026.2.12. Recommend **v2026.4.24+** for the latest plugin metadata, diagnostics, realtime media, browser automation, model/provider, and channel reliability updates. Run `openclaw status` to check.
...
## Breaking Changes to Watch For (v2026.3.x through v2026.4.24)
...
21. **Cron runtime state is split from definitions** (v2026.4.20) — execution state moves to `jobs-state.json`; keep `jobs.json` for authored/git-tracked job definitions and let `openclaw doctor --fix` repair malformed ids.
22. **Codex onboarding no longer imports CLI auth material** (v2026.4.22) — use browser login or device pairing for OpenAI Codex rather than expecting `~/.codex` OAuth material to be copied into OpenClaw auth stores.
23. **Chat `/models add` is deprecated** (v2026.4.24) — use model auth/setup and config workflows instead of relying on chat-side model writes.
24. **Plugin SDK tool-result transform compatibility removed** (v2026.4.24) — bundled tool-result rewrites must use `api.registerAgentToolResultMiddleware(...)` with declared `contracts.agentToolResultMiddleware` targets.
...
1. **Cron state split and repair** (v2026.4.20) — cron runtime execution state now lives in `jobs-state.json`, while `jobs.json` remains stable for authored definitions; `openclaw doctor --fix` can repair malformed persisted job ids.
2. **Diagnostics export and OTEL expansion** (v2026.4.22-v2026.4.24) — payload-free stability recording, support-ready diagnostics exports, and expanded low-cardinality OTEL spans cover runs, model calls, tool execution, exec processes, and logs.
3. **OpenAI Codex auth and model routing updates** (v2026.4.22-v2026.4.24) — use browser/device-pairing auth, current `openai-codex/gpt-5.5` catalog behavior, and native Codex harness status/approval integration.
4. **Plugin metadata/runtime dependency hardening** (v2026.4.24) — startup, doctor, setup, provider discovery, and model listing rely more on cold manifest/catalog metadata and external runtime-dependency repair for packaged installs.
5. **Bundled Google Meet participant plugin** (v2026.4.24) — adds personal Google auth, Chrome/Twilio realtime sessions, paired-node Chrome support, artifact/attendance exports, and tab recovery tooling.
6. **Realtime voice and TTS expansion** (v2026.4.22-v2026.4.24) — Talk, Voice Call, Google Meet, xAI, Gemini Live, Gradium, OpenRouter, and several STT/TTS providers gain broader realtime/media support.
7. **Browser automation upgrades** (v2026.4.24) — coordinate clicks, `openclaw browser click-coords`, `browser.actionTimeoutMs`, per-profile headless/executable overrides, stable tab handles, and deeper browser doctor diagnostics improve host/browser-node operations.
8. **Model/catalog changes** (v2026.4.22-v2026.4.24) — DeepSeek V4 Flash/Pro, Tencent Cloud, OpenAI image generation through Codex OAuth, OpenRouter image support, Kimi K2.6 defaults, and manifest-sourced model rows improve listing/onboarding without runtime imports.
9. **Security hardening** (v2026.4.20-v2026.4.24) — new fixes cover workspace `.env` runtime-control blocking, device-pairing scope limits, owner-only MCP/tool access, Teams token audience checks, mobile cleartext pairing constraints, plugin setup path lookup, and SecretRef-backed webhook hot reload.
10. **`v2026.4.25-beta.*` watch note** — prereleases add `/tts latest`, chat-scoped TTS controls, `openclaw plugins registry`, `openclaw browser start --headless`, richer OTEL/Prometheus diagnostics, and plugin cold-registry changes; treat as beta until a stable tag ships.
...
Claude 4.7 models default to adaptive thinking where supported. Override if needed:
