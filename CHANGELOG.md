...
### Added
- Added release-alignment coverage for upstream stable `v2026.4.20` and `v2026.4.21` across core skill/docs, including new guidance for cron runtime state split (`jobs-state.json`) and owner-command identity enforcement tightening when `enforceOwnerForCommands=true`.
- Added troubleshooting entries for post-upgrade owner-only command authorization failures, Slack thread alias drift in runtime sends, and cron `jobs.json` churn expectations after the runtime state-file split.
- Added security-checklist hardening entries for `v2026.4.20` workspace `.env` `OPENCLAW_*` runtime-control key blocking and `v2026.4.21` owner-command identity fallback removal.
- Added channel/login notes for `v2026.4.21` Slack outbound `threadTs` alias preservation and explicit owner identity mapping expectations (`commands.ownerAllowFrom`) under owner-enforced command policies.
