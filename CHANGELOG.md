...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.4` through `v2026.5.7`, including the v2026.5.6 revert of v2026.5.5's destructive Codex OAuth route rewrite, v2026.5.7 OAuth-only recovery in `doctor --fix`, Docker container capability hardening (`NET_RAW`/`NET_ADMIN` drop, `no-new-privileges`), `openclaw channels list` channel-only scoping with `--all` for bundled/catalog inventory, `openclaw models auth list` for inspecting saved per-agent auth profiles, `cron list --json` / `cron show --json` computed `status` field, and Tavily `tavily_search`/`tavily_extract` bundled tools.
- Added SKILL.md and CLI reference coverage for `openclaw gateway status --deep`, `openclaw doctor --deep` supervisor restart handoff visibility, `openclaw proxy validate --apns-reachable`, `openclaw channels status --probe`/`channels capabilities` Discord voice permission auditing, and `openclaw models set` Codex OAuth recovery workflow.
- Added new config-key coverage for `streaming.progress.render: "rich"` (Slack Block Kit progress drafts), `agents.defaults.toolProgressDetail: "raw"`, `streaming.preview.commandText`/`streaming.progress.commandText` `"status"` mode, `voice.captureSilenceGraceMs`, `agents.defaults.subagents.archiveAfterMinutes`, and `tools.loopDetection.postCompactionGuard.windowSize`.
- Added `OPENCLAW_DEBUG_PROXY_ALLOW_DIRECT_CONNECT_WITH_MANAGED_PROXY` environment-variable coverage for opt-in direct upstream forwarding through debug proxy/CONNECT requests while managed proxy mode is active.
- Added LINE channel guidance documenting v2026.5.5+ `dmPolicy: "open"` validation tightening, IRC channel guidance documenting raw TCP/TLS egress note from v2026.5.4, Telegram channel guidance for `accessGroup:*` sender allowlists added in v2026.5.7, and Discord transport-degradation visibility in `channels status` / `status --deep`.
- Added troubleshooting recipes for v2026.5.5 Codex OAuth route rewrite recovery, Discord transport degradation diagnosis, Discord voice capability auditing via `channels capabilities`, Telegram `accessGroup:*` allowlist enablement, cron `payload.model` repair via `doctor --fix`, `OPENCLAW_GATEWAY_TOKEN` shadowing detection, and supervisor restart handoff visibility.
- Added security-checklist hardening entries for v2026.5.4 Windows install-root env validation (`SystemRoot`/`WINDIR`, `icacls.exe`/`whoami.exe`/`reg.exe`/`cmd.exe` resolution), browser SSRF tab-route hardening, debug-proxy direct-connect lockdown under managed proxy mode, direct APNs proxy routing, tree-sitter shell command explainer foundation, per-runtime sandbox registry shards, Codex `/codex` command output sanitization and fail-closed parsing, and trust-gated plugin install scanner; v2026.5.5 Docker container capability drops + `no-new-privileges`, doctor heartbeat-poison repair, and `OPENCLAW_GATEWAY_TOKEN` shadowing warning; v2026.5.7 native command owner enforcement, Active Memory admin scope, auto-reply skill tool authorization gating, and Tavily SecretRef credential resolution.
