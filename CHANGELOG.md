...
### Added
- Added release-alignment coverage for upstream stable `v2026.4.5`, including the new breaking config-alias migration (`talk.voiceId`/`talk.apiKey`, `agents.*.sandbox.perSession`, `browser.ssrfPolicy.allowPrivateNetwork`, `hooks.internal.handlers`, legacy `allow` toggles) and explicit `openclaw doctor --fix` remediation guidance.
- Added core skill and CLI coverage for `v2026.4.5` operational additions: built-in `video_generate` and `music_generate` tools, `openclaw plugins install --force`, bundled provider expansion, and per-channel `contextVisibility` controls.
- Added troubleshooting guidance for post-upgrade validation failures caused by removed legacy alias keys in `v2026.4.5`, with migration and re-validation workflow examples.
- Added security checklist hardening notes for `v2026.4.5` fail-closed tool allowlist behavior, browser SSRF redirect bypass blocking, and tightened plugin-route/device scope enforcement.
...
### Changed
- Updated recommended OpenClaw target version across the plugin from `v2026.4.2+` to `v2026.4.5+` while preserving minimum safe baseline at `v2026.3.1`.
