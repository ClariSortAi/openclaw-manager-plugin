...
### Added
- Added release-alignment coverage for upstream stable `v2026.4.5` through `v2026.4.12`, including breaking-change guidance for legacy config-alias removal (`openclaw doctor --fix` + `openclaw config validate`) and startup fail-closed behavior for placeholder gateway credentials.
- Added CLI reference coverage for the `openclaw infer ...` command hub (v2026.4.7) and `openclaw exec-policy show|preset|set` workflow (v2026.4.10), plus `models.providers.*.request.allowPrivateNetwork` config guidance for trusted self-hosted OpenAI-compatible endpoints.
- Added troubleshooting coverage for missing `openclaw infer`/`openclaw exec-policy` commands on older runtimes, v2026.4.5 alias-removal validation failures, and v2026.4.12 placeholder-credential startup rejection.
- Added security-checklist hardening notes for v2026.4.7-v2026.4.12 fetch/body redirect stripping, expanded browser SSRF checks, workspace dotenv control-var blocking, tightened exec safe-bin/approver validation, and gateway example-credential fail-closed startup behavior.
