...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.28` and prerelease watch `v2026.6.1-beta.2`, including Node v22.19 guidance, typed plugin authoring commands, Workboard/Codex Supervisor/Copilot/Tokenjuice surfaces, ClawPDF encrypted PDF extraction, Claude Opus 4.8/provider coverage, channel reliability fixes, Skill Workshop beta watch, SecretRef plugin manifests, SQLite-backed channel/plugin state, and bounded diagnostics/CI/provider paths.
- Added troubleshooting coverage for current stable plugin package validation/build failures and stricter Browser input/dialog/timeout behavior.
- Added security-checklist entries for v2026.5.28 hardening around phone-control/directive authorization, Teams service URL trust, workspace dotenv provider isolation, browser token/input bounds, strict config/input parsing, and plugin package/API compatibility.
...
- Updated Node.js minimum guidance from **v22.14.0+** to **v22.19.0+** to match the current stable runtime floor while still recommending Node 24 for new installs.
