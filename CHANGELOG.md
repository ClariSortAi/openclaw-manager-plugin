...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.19` through `v2026.5.22` and prerelease watch `v2026.5.25-beta.1`, including Node `22.19.0+` guidance, typed plugin authoring commands, shared skills, browser dialog/evaluate controls, HTTPS proxy CA trust, Meeting Notes CLI access, Discord voice following/profile context, xAI device-code auth, OpenRouter routing params, Gateway startup/performance improvements, and latest security/provenance hardening.
- Added troubleshooting coverage for v2026.5.19-v2026.5.22 Node engine failures, plaintext secret doctor warnings, browser modal-dialog blocking, and Meeting Notes/Discord voice capture repair workflows.
- Added security-checklist entries for v2026.5.20-v2026.5.22 hardening around skill-wrapper exec approval removal, secret-file symlink rejection, plaintext secret diagnostics, reduced sub-agent bootstrap context, published package shrinkwraps, Docker token log redaction, diagnostics identifier scrubbing, workspace-provider plugin setup trust, Diffs toolbar XSS removal, and bounded tool-policy audit logs.
...
### Changed
- Updated recommended OpenClaw target version across the plugin from `v2026.5.12+` to `v2026.5.22+` while preserving minimum safe baseline at `v2026.3.1`.
- Updated Node.js minimum guidance from **v22.14.0+** to **v22.19.0+** to reflect upstream runtime floor changes in `v2026.5.19` while still recommending Node 24 for new installs.
