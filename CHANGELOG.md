...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.18` through `v2026.5.27` and prerelease watch `v2026.5.28-beta.2`, including transcript/meeting-notes workflows, named model auth profiles, OpenAI-compatible embeddings, Pixverse/DeepInfra/VLLM provider coverage, channel reaction approvals, Telegram durable action delivery, Slack/Teams hardening, Codex app-server recovery, Gateway/status performance, and latest content-boundary security fixes.
- Added troubleshooting coverage for v2026.5.22-v2026.5.27 auth/profile confusion, no-auth Tailscale exposure rejection, Telegram `sendMessage` action delivery, meeting-notes command availability, and narrowed sub-agent bootstrap context.
- Added security-checklist entries for v2026.5.22-v2026.5.27 hardening around memory prompt-injection filtering, remote auth rate limiting, Browser snapshot SSRF checks, external content wrapping, stale device-token rejection, plugin model regex validation, Diffs toolbar XSS, group prompt boundaries, side-effecting wrapper blocks, unsafe Node runtime env overrides, Tailscale no-auth exposure rejection, node/device-role approval authority, and Teams service URL trust.
