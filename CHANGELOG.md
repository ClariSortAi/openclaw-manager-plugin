...
- Added v2026.3.11 breaking-change guidance for tightened isolated cron delivery, including explicit post-upgrade `openclaw doctor --fix` migration coverage.
- Added v2026.3.11 security coverage for trusted-proxy WebSocket/browser-origin enforcement (GHSA-5wcw-8jjv-m286) and secret/archive symlink hardening.
- Added v2026.3.11 CLI/runtime notes for `OPENCLAW_CLI` child-process environment marking and cron migration behavior.
...
- Updated recommendation language across skill, security, and troubleshooting docs from v2026.3.8+ to v2026.3.11+ while preserving v2026.3.1 as the minimum safe baseline.
