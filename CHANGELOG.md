...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.4` through `v2026.5.7`, including `openclaw models auth list`, the v2026.5.7 `openclaw channels list` re-scoping plus `--all` flag, cron `list --json`/`show --json` computed `status` field, post-compaction loop guard (`tools.loopDetection.postCompactionGuard.windowSize`), Slack rich Block Kit progress drafts (`streaming.progress.render: "rich"`), `streaming.preview.commandText`/`streaming.progress.commandText` controls, `agents.defaults.toolProgressDetail` compact-vs-raw setting, Google Meet agent/bidi realtime split, `voice.captureSilenceGraceMs` tuning, Discord voice permission audit via `openclaw channels capabilities`/`status --probe`, supervisor restart-handoff visibility in `gateway status --deep` and `doctor --deep`, `openai/chat-latest` ChatGPT Instant alias, Telegram `accessGroup:*` allowlists, WhatsApp LID forward routing, xAI Grok native-Responses thinking guard, and Codex/`openai-codex/*` route recovery in `doctor --fix`.
- Added troubleshooting coverage for the v2026.5.5 `openai-codex/*` → `openai/*` regression (recovery via `openclaw models set openai-codex/gpt-5.5 && openclaw config validate`, full repair in v2026.5.7 `doctor --fix`), v2026.5.4 Windows loopback-only `127.0.0.1` binding behavior, v2026.5.7 `openclaw channels list` output narrowing, Discord voice permission/silence-grace tuning, xAI Grok native-Responses thinking errors, `compaction_loop_persisted` aborts, and cron `payload.model` doctor repair.
- Added security-checklist hardening entries for v2026.5.4 Docker compose container hardening (`NET_RAW`/`NET_ADMIN` drop, `no-new-privileges`), browser SSRF current-tab enforcement, restrictive `plugins.allow` bundled-discovery scoping (`plugins.bundledDiscovery: "compat"` migration), Windows registry/`cmd.exe` helper trust hardening, WebChat short-lived media tickets, exec-approval `env -S`/`env -P` carrier detection, debug proxy direct-forward gating, and QQBot command authorization fixes; plus v2026.5.7 Active Memory admin-scope toggles, auto-reply skill dispatch authorization gating, native command owner enforcement, and Codex approval pre-guardian hook default change.
- Added channel-setup coverage for v2026.5.7 Telegram `accessGroup:*` allowlists, Discord voice channel permission auditing and silence-grace tuning, Slack rich Block Kit progress drafts and improved Socket Mode reconnect logs (v2026.5.4-v2026.5.5), LINE `dmPolicy: "open"` wildcard requirement (v2026.5.5), and WhatsApp LID forward routing/canonical phone-id allowlist normalization.
...
- Updated breaking-change banner header from "v2026.3.x through v2026.5.3" to "v2026.3.x through v2026.5.7" and appended v2026.5.4-v2026.5.7 entries (channels list re-scoping, restrictive `plugins.allow` bundled-discovery scoping, Windows loopback-only binding, Google Meet realtime agent/bidi split, Codex approval pre-guardian default change).
