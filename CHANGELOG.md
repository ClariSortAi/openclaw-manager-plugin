...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.18` through `v2026.5.22`, including Node `v22.19.0+` runtime guidance, typed plugin authoring commands, global skill installs, browser dialog/evaluate controls, Meeting Notes, policy/plugin notes, xAI/OpenRouter/Codex auth updates, Discord voice follow behavior, and channel/provider reliability fixes.
- Added troubleshooting coverage for Node engine-floor update failures, stale official plugin peer links/bundled records, global skill management flags, typed plugin authoring command availability, and browser modal-dialog handling.
- Added security-checklist entries for v2026.5.18-v2026.5.22 hardening around skill exec allowlist removal, credential symlink rejection, plaintext secret warnings, shrinkwrapped OpenClaw-owned npm packages, workspace provider-plugin trust gating, sub-agent bootstrap minimization, exec denial log redaction, Docker token redaction, diffs toolbar XSS hardening, and node service token isolation.
