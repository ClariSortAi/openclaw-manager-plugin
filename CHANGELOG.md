...
### Added
- Added release-alignment coverage for upstream stable `v2026.4.20` and `v2026.4.21`, including owner-command identity enforcement changes (`enforceOwnerForCommands` + `commands.ownerAllowFrom` behavior), cron runtime state split guidance (`jobs-state.json` vs `jobs.json`), and Slack runtime `threadTs` alias reliability notes.
- Added troubleshooting guidance for v2026.4.21 regressions: owner-enforced command authorization failures under wildcard/empty-owner fallback assumptions, threaded Slack runtime reply routing drift, and packaged-install bundled runtime dependency repair workflows via `openclaw doctor --fix`.
- Added security-checklist hardening entries for v2026.4.21 owner-command identity enforcement and bundled plugin-runtime doctor repair coverage for packaged installs.
