...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.4`-`v2026.5.7`, including the `openclaw models auth list` command, the v2026.5.7 `openclaw channels list` scope split (with `--all` for bundled/catalog channels), Slack rich Block Kit progress drafts (`streaming.progress.render: "rich"`), the post-compaction loop guard (`tools.loopDetection.postCompactionGuard.windowSize`), WhatsApp LID forward mapping and digit-only allowlist canonicalization, Telegram `accessGroup:*` sender allowlists, Discord IPv4 preference, Discord voice permission audits and `voice.captureSilenceGraceMs`, Discord `/steer` plain-text routing, Google Meet realtime voice bridge, `openclaw doctor --deep`/`gateway status --deep` supervisor restart visibility, bounded `openclaw sessions list --limit`/JSON pagination metadata, `plugins.bundledDiscovery: "compat"` migration, `openai/chat-latest` direct API-key alias, and cron `--json` computed status fields.
- Added Codex OAuth route recovery guidance covering the v2026.5.5 `doctor --fix` regression that rewrote valid `openai-codex/*` routes to `openai/*`, the v2026.5.6 revert, and the v2026.5.7 `doctor --fix` preservation/recovery behavior.
- Added security-checklist hardening entries for Windows loopback `127.0.0.1`-only bind, bundled provider discovery default tightening, iOS pairing transport guards, Control UI media-fetch ticketed URLs, compaction loop guard defaults, expanded browser SSRF checks on existing-session screenshots, WhatsApp allowlist canonicalization, Docker Compose `NET_RAW`/`NET_ADMIN` drop with `no-new-privileges`, LINE `dmPolicy: "open"` validation, Telegram polling-watchdog liveness, native command owner enforcement, Active Memory admin-scope toggles, before-tool-call gating for inline skill dispatch, websocket scope clamping, pair-command pairing-scope requirement, and QQBot framework command isolation.
- Added troubleshooting coverage for the v2026.5.5 Codex OAuth route rewrite recovery, Discord `/steer` plain-text command authorization, Discord voice permission audits and capture-silence tuning, Discord `discord:channel:<id>` provider-prefixed targets, WhatsApp LID ghost-chat repair, WhatsApp digit-only allowlist canonicalization, LINE `dmPolicy: "open"` validation, Telegram `accessGroup:*` allowlist support, `compaction_loop_persisted` aborts, bounded `openclaw sessions list` output, the v2026.5.7 `openclaw channels list` scope split, `plugins.bundledDiscovery: "compat"` migration recovery, supervisor restart visibility in `--deep` status, and `OPENCLAW_GATEWAY_TOKEN` shadow detection.
- Added channel-setup notes covering Discord voice permission audits and capture-silence grace, Discord `/steer` plain-text routing, IRC direct-egress disclosure, WhatsApp LID forward mappings, Telegram `accessGroup:*` sender allowlists, Feishu native topic starter thread hydration, and LINE `dmPolicy: "open"` validation requirements.
