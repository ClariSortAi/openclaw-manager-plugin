...
### Added
- Added prerelease-watch coverage for upstream `v2026.5.16-beta.2`, including xAI Grok OAuth login, `openclaw cron run --wait`, exact `cron runs --run-id` filtering, localized onboarding flows, Telegram `messages.groupChat.ambientTurns`, skills/context-engine caching, and beta doctor/update/input hardening notes.
- Added troubleshooting and security-watch guidance for beta cron automation blocking, media MIME sniffing, plugin metadata validation, malformed persisted-state repair, and Codex native tool-policy enforcement.
