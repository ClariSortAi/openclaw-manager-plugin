...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.5`, including Feishu/LINE/Matrix/Discord channel fixes, progress/status runtime visibility, `doctor --fix` session and Codex-route repair, official plugin update resilience, session cleanup pruning, and xAI/Fireworks thinking-profile guidance.
- Added security/troubleshooting coverage for `v2026.5.5` guarded Windows exec-approval writes, Docker Compose capability hardening, LINE `dmPolicy: "open"` wildcard validation, `OPENCLAW_GATEWAY_TOKEN` shadow warnings, and official plugin SDK peer-link repair paths.
- Added release-alignment coverage for upstream stable `v2026.5.4`, including Google Meet/Voice Call realtime bridge improvements, `openclaw models auth list`, bounded session-listing guidance, rich progress streaming controls, official plugin repair hints, external channel SecretRef contract fixes, and Discord/Slack/IRC channel reliability notes.
- Added security/troubleshooting coverage for `v2026.5.4` Windows gateway and host-env hardening, browser current-tab SSRF enforcement, plugin package-boundary validation, channel command authorization hardening, and large-session automation polling safeguards.
...
### Changed
- Updated recommended OpenClaw target version across the plugin from `v2026.5.4+` to `v2026.5.5+` while preserving minimum safe baseline at `v2026.3.1`.
- Updated README/plugin metadata for this docs release alignment (`1.3.10`).
- Updated recommended OpenClaw target version across the plugin from `v2026.5.3+` to `v2026.5.4+` while preserving minimum safe baseline at `v2026.3.1`.
