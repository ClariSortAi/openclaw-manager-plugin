...
### Added
- Added release-alignment coverage for upstream stable `v2026.6.1` and prerelease watch `v2026.6.5-beta.6`, including Skill Workshop governance, Workboard orchestration, SQLite-backed plugin/auth/channel state, typed plugin authoring commands, Codex/OpenAI recovery, MiniMax M3/provider catalog updates, external Copilot/Tokenjuice plugins, Parallel web-search beta watch, MCP rich-content coercion, Google Chat native approvals, Matrix voice/thread beta notes, and latest channel/runtime reliability fixes.
- Added troubleshooting guidance for current stable Node engine failures, typed plugin package validation, stale SQLite-backed plugin install ledgers, legacy cron/runtime state migrations, and interrupted Codex/OpenAI session recovery.
- Added security-checklist coverage for v2026.5.18-v2026.6.1 hardening around Node v22.19.0+ engine checks, runtime-ready plugin validation, SQLite-backed install ledgers, owner-only HTTP tool gating, unsafe lifetime/timeout parsing, and beta-watch MCP/transcript redaction hardening.
