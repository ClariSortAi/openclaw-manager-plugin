...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.18` through `v2026.5.27`, including Node 22.19+ runtime guidance, Gateway/startup observability, Policy plugin, Meeting Notes/transcripts, Discord voice/Talk reliability, xAI device-code OAuth, OpenRouter routing, Telegram forum-topic reliability, named model auth profiles, reaction approvals, OpenAI-compatible embedding providers, Pixverse video generation, DeepInfra catalog browsing, and latest channel delivery hardening.
- Added troubleshooting coverage for late-May provider/model catalog issues, xAI headless login, Codex app-server auth/resume/tool recovery, Telegram durable/topic-aware delivery, Discord voice/Talk follow-ups, and the raised Node runtime floor.
- Added security-checklist entries for v2026.5.26-v2026.5.27 hardening around explicit memory-store prompt filtering, remote auth rate limiting, external content and system-event boundaries, sender allowlists, invalidated device-token rejection, durable/reaction approvals, transcript metadata escaping, safe plugin model regex compilation, group prompt boundary enforcement, hostname normalization, unsafe Node env override blocking, no-auth Tailscale rejection, Teams service URL blocking, config-write origin policy, and admin-only node/device-role approvals.
...
- Updated cron command example model override from `openai/gpt-5.4` to `openai/gpt-5.5`.
