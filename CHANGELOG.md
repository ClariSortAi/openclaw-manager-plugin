...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.18` through `v2026.6.8`, including Node 22.19.0+ guidance, typed plugin authoring commands, Skill Workshop, Workboard, Meeting Notes, SQLite-backed state migrations, Parallel web search, usage footers, richer Telegram/WhatsApp delivery, Matrix/Google Chat/Feishu updates, GLM-5.2, Claude Haiku 4.5, Claude Opus 4.8, MiniMax M3, DeepInfra/NVIDIA provider catalog updates, and official GitHub Copilot/Tokenjuice/Codex Supervisor plugin split-outs.
- Added troubleshooting coverage for v2026.6.x SQLite state migrations, Telegram rich-message rendering, explicit key-free web-search provider opt-in, Gemini CLI auth-profile isolation, and expanded official plugin dependency repair.
- Added security-checklist entries for v2026.5.18-v2026.6.8 hardening around skill execution compatibility removal, secret-file symlink rejection, group prompt boundaries, no-auth Tailscale exposure, side-effecting command/Node env blocking, Teams service URL trust, device/node role admin gating, browser token expiry after auth rotation, MCP redirect guarding, admin-only HTTP override surfaces, and the patched Hono runtime.
...
- Updated Node.js minimum guidance from **v22.14.0+** to **v22.19.0+** while still recommending Node 24 for new installs.
