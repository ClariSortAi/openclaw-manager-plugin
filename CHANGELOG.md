...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.18` and prerelease watch `v2026.5.19-beta.2`, including Node 22.19 runtime-floor guidance, typed tool-plugin CLI (`plugins init|validate|build`), proxy TLS CA config, Docker/Podman image package hooks, Telegram/Discord delivery reliability, Codex/OpenAI runtime recovery, Gateway startup/protocol diagnostics, and latest robustness/security rollups.
- Added troubleshooting coverage for v2026.5.18 Node engine-floor failures, npm/root-owned package update recovery, externalized plugin repair improvements, and Discord/Telegram progress/final-reply delivery regressions.
- Added security-checklist entries for v2026.5.18 hardening around trusted-proxy local-direct password fallback clarification, gateway auth/log redaction, SSRF/private-network guardrails, exec approval realpath binding, Docker non-loopback fail-closed behavior, and malformed JSON/base64/URL parsing robustness.
...
### Changed
- Updated recommended OpenClaw target version across the plugin from `v2026.5.12+` to `v2026.5.18+` while preserving minimum safe baseline at `v2026.3.1`.
- Updated Node.js minimum guidance from **v22.14.0+** to **v22.19.0+ on Node 22** to reflect upstream runtime floor changes in `v2026.5.18`, while still recommending Node 24 for new installs.
