...
### Added
- Added release-alignment coverage for upstream stable `v2026.4.20` through `v2026.4.24`, including cron `jobs-state.json` guidance, diagnostics export/OTEL notes, Google Meet participant plugin coverage, Voice Call/Google Meet readiness commands, browser automation CLI/config additions, plugin metadata/runtime-dependency repair guidance, and current model/catalog changes.
- Added troubleshooting guidance for `v2026.4.20-v2026.4.24` gaps: plugin metadata/runtime dependency repair, browser automation stale-tab/action-timeout recovery, Google Meet OAuth/browser recovery, Voice Call smoke checks, cron state split migration, and `OPENCLAW_*` workspace `.env` blocking.
- Added security-checklist entries for `v2026.4.20-v2026.4.24` hardening: runtime-control `.env` blocking, paired-device scope restrictions, websocket broadcast scoping, gateway config mutation allowlist, Teams audience checks, SecretRef webhook hot reload, plugin setup path lookup hardening, owner-only MCP/tool access, and browser admin authority.
- Added `v2026.4.25-beta.*` watch notes for beta-only TTS/plugin-registry/browser/diagnostics changes without treating them as stable guidance.
...
- Updated remaining Anthropic model examples from Claude 4.6 to 4.7 across manager and onboarding docs.
