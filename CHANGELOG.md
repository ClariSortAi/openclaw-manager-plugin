...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.4` through `v2026.5.7`, including the new Google Meet `realtime.strategy: "agent"` default and `mode: "agent"/"bidi"` voice paths, Slack Block Kit progress drafts (`streaming.progress.render: "rich"`), preview/progress `commandText: "status"` controls, raw tool-progress mode (`agents.defaults.toolProgressDetail: "raw"`), post-compaction loop guard (`tools.loopDetection.postCompactionGuard.windowSize`), `agents.defaults.subagents.archiveAfterMinutes` retention, `plugins.bundledDiscovery: "compat"` legacy migration, and Plugin SDK `before_agent_finalize` retry semantics.
- Added CLI reference coverage for `openclaw models auth list [--provider <id>] [--json]` (v2026.5.4), `openclaw models set <provider/model>` (v2026.5.6+ Codex recovery), `openclaw channels list --all` channel-only output with `openclaw channels capabilities` and `openclaw channels status --probe` Discord voice-permission audits (v2026.5.7), `cron list --json` / `cron show --json` computed-status output (v2026.5.7), `openclaw sessions list --limit <n|all>` JSON pagination metadata (v2026.5.4), and `openclaw doctor --deep` / `openclaw gateway status --deep` supervisor restart handoff visibility (v2026.5.5).
- Added troubleshooting coverage for OpenAI Codex OAuth route regression in v2026.5.5 (recovered in v2026.5.6/v2026.5.7 via `openclaw models set openai-codex/gpt-5.5`), iOS pairing `ws://` transport tightening, LINE `dmPolicy: "open"` validation, Discord voice-permission audits and IPv4 startup preference, cron `payload.model` doctor repair, cron `timeoutSeconds` LLM idle watchdog behavior, agent post-compaction loop guard tuning, and the `openclaw channels list` channel-only move.
- Added channel/setup notes for v2026.5.4-v2026.5.7 WhatsApp LID forward routing for proactive phone-number sends and inbound group visible-reply mode default; Telegram `accessGroup:*` allowlist precedence; Discord voice capture silence-grace defaults and `voice.captureSilenceGraceMs` tuning; Slack rich Block Kit progress drafts, preview/progress command-text hiding controls, and structured socket-mode retry error reporting.
- Added security-checklist hardening entries for v2026.5.4 tab-scoped browser SSRF enforcement, Windows install-root env hardening (`SystemRoot`/`WINDIR`/`LOCALAPPDATA`), unbound WebSocket auth scope clamping, pairing scope enforcement, ZaloUser startup name-matching gating, QQBot framework command authorization preservation, Codex chat-surface sanitization and bound-turn integrity, Control UI media ticketing, direct APNs egress through managed proxy, and debug-proxy direct-forward lockout; v2026.5.5 Docker compose container hardening (`NET_RAW`/`NET_ADMIN` drop, `no-new-privileges`), iOS pairing transport rules, and LINE policy validation; v2026.5.7 Active-Memory admin scope, auto-reply tool dispatch authorization, native command owner enforcement, and Tavily SecretRef resolution from active runtime snapshot.
...
- Added security-checklist hardening entries for file-transfer default-deny path policy, source-only plugin package rejection, invalid config fail-closed behavior, restricted profile widening removal, and plugin-isolated state stores.
...
- Added channel/auth notes covering Slack interactive sender verification tightening and Teams SSO allowlist enforcement changes introduced in `v2026.4.14`.
...
### Changed
- Updated recommended OpenClaw target version across the plugin from `v2026.5.3+` to `v2026.5.7+` while preserving minimum safe baseline at `v2026.3.1`. The bump captures the v2026.5.6 revert of the v2026.5.5 Codex OAuth doctor regression and the v2026.5.7 explicit preservation of working `openai-codex/*` PI routes during repair.
