...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.18` through `v2026.6.1` and prerelease watch `v2026.6.2-beta.1`, including Skill Workshop review flows, Workboard orchestration, GitHub Copilot/Tokenjuice external plugins, Codex Supervisor, SQLite-backed plugin/channel/session state, MiniMax M3, Claude Opus 4.8, ClawPDF/encrypted PDF extraction, and broader channel/provider runtime bounds.
- Added troubleshooting coverage for v2026.5.28-v2026.6.1 official external plugin loader/ledger repair and v2026.6.1 stale disabled-skill SecretRef snapshot recovery.
- Added security-checklist entries for v2026.5.28-v2026.6.1 hardening around Teams service URL trust, policy/conformance checks, phone-control/directive authorization, SecretRef plugin manifests, stale disabled skill snapshots, and runtime/input timeout bounds.
