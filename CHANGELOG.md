...
### Added
- Added release-alignment coverage for upstream stable `v2026.4.25` and `v2026.4.26`, including expanded TTS/voice controls, persisted plugin registry workflows, OpenTelemetry/Prometheus diagnostics, browser automation commands, migration importers, Cerebras provider support, Matrix E2EE setup, memory/compaction improvements, and install/update hardening.
- Added CLI reference coverage for `openclaw plugins registry --refresh`, `openclaw migrate`, `openclaw browser doctor --deep`, `openclaw browser start --headless`, `openclaw models auth setup-token --provider cerebras`, Matrix encryption setup, and new operational environment variables.
- Added troubleshooting coverage for stale persisted plugin registries, invalid plugin config quarantine, `openclaw update` mixed-version/stale-file verification failures, explicit same-agent sub-agent allowlist enforcement, and large transcript preflight compaction.
- Added security-checklist hardening entries for `v2026.4.25-v2026.4.26`: low-cardinality telemetry redaction boundaries, device-token rotation response hardening, stricter sub-agent allowlist enforcement, logging/session redaction, plugin install symlink/test-scan hardening, route probe reservations, and plugin config quarantine.
