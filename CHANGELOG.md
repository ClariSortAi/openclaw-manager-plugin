...
### Added
- Added release-alignment coverage for upstream stable `v2026.5.28` and prerelease watch `v2026.5.30-beta.1`, including active-subagent status visibility, ClawPDF/encrypted PDF extraction, Claude Opus 4.8/Fal Krea/NVIDIA/MiniMax/voice catalog coverage, GitHub Copilot/Tokenjuice official plugin packaging, Workboard and policy checks, ClawHub trust surfaces, strict config/browser/channel input parsing, and Skill Workshop/SecretRef manifest prerelease tracking.
- Added troubleshooting coverage for v2026.5.28 Teams service URL/DNS trust failures, recurring cron retry behavior after transient model rate limits, stricter browser payload validation, encrypted PDF extraction, and canonical model auth profile migration.
- Added security-checklist entries for v2026.5.28 ClawHub trust surfaces, strict OAuth/token/retry/timeout parsing, browser/channel input hardening, and Teams attachment/service URL trust checks.
- Expanded `user-login-mechanism.md` with OpenAI ChatGPT/Codex vs API-key login behavior, named model auth profile verification, and newer provider auth commands.
- Added release-alignment coverage for upstream stable `v2026.5.27` and prerelease watch `v2026.5.28-beta.4`, including Node 22.19 runtime guidance, typed plugin authoring commands, HTTPS proxy TLS notes, browser dialog handling, Meeting Notes/transcripts, Policy plugin coverage, reaction approvals, named model auth profiles, OpenAI-compatible embeddings, DeepInfra/Pixverse provider updates, Codex app-server recovery, and current security/content-boundary hardening.
- Added troubleshooting coverage for current stable Node runtime floor failures, browser modal-dialog blocks, named model auth profile ambiguity, and Meeting Notes/transcript setup gaps.
- Added security-checklist entries for v2026.5.18-v2026.5.27 hardening around default remote auth rate limiting, Browser snapshot SSRF checks, external-content boundaries, unsafe command/runtime blocking, no-auth Tailscale rejection, admin-gated node/device approvals, secret symlink fail-closed behavior, plaintext secret warnings, plugin regex validation, and Docker token-output hardening.
...
- Added security-checklist entries for v2026.5.5-v2026.5.12 hardening around native command ownership, Active Memory admin scope, inline skill authorization, Docker Compose capabilities, structured SecretRef provider auth, Windows sandbox home roots, delegated-session tool restrictions, transcript redaction, approval callback authorization, browser CDP relay auth, hook/gateway authority, and parser/input hardening.
...
### Changed
- Updated recommended OpenClaw target version across the plugin from `v2026.5.27+` to `v2026.5.28+` while preserving minimum safe baseline at `v2026.3.1`.
- Updated README/version metadata for this docs release alignment (`1.3.11`).
- Updated recommended OpenClaw target version across the plugin from `v2026.5.12+` to `v2026.5.27+` while preserving minimum safe baseline at `v2026.3.1`.
- Updated Node.js minimum guidance from **v22.14.0+** to **v22.19.0+** to reflect the upstream runtime floor raised in `v2026.5.18-v2026.5.19`.
